...
// import { transferEdCoin } from './Transaction';
// Optional: import an icon library
// import { FaCoins } from 'react-icons/fa';

// --- Mock Data --- (Replace with API data later) ---
const rewardsData = [
    { id: 1, name: 'Free Coffee Voucher', description: 'Get a voucher for one free coffee at participating cafes.', cost: 50, imageUrl: 'https://via.placeholder.com/300x200/F3F4F6/6B7280?text=Coffee', category: 'Vouchers' },
    { id: 2, name: 'Online Course Discount (10%)', description: '10% off your next online course purchase on Platform X.', cost: 100, imageUrl: 'https://via.placeholder.com/300x200/F3F4F6/6B7280?text=Discount', category: 'Education' },
    { id: 3, name: 'EdCoin T-Shirt', description: 'Exclusive EdCoin branded t-shirt (select size).', cost: 250, imageUrl: 'https://via.placeholder.com/300x200/F3F4F6/6B7280?text=T-Shirt', category: 'Merchandise' },
    { id: 4, name: '$5 Donation to Charity', description: 'We\'ll donate $5 to a selected educational charity.', cost: 75, imageUrl: 'https://via.placeholder.com/300x200/F3F4F6/6B7280?text=Donation', category: 'Charity' },
    { id: 5, name: 'Premium Content Access (1 Month)', description: 'Unlock premium articles and tutorials for one month.', cost: 150, imageUrl: 'https://via.placeholder.com/300x200/F3F4F6/6B7280?text=Premium', category: 'Access' },
    { id: 6, name: 'Laptop Sticker Pack', description: 'Cool stickers for your laptop featuring EdCoin designs.', cost: 30, imageUrl: 'https://via.placeholder.com/300x200/F3F4F6/6B7280?text=Stickers', category: 'Merchandise' },
];
// --- End Mock Data ---
...
const RewardsPage = () => {
  ...
  const handleRedeem = (reward) => {
    ...
    if (userCoins >= reward.cost) {
      console.log(`Attempting to redeem: ${reward.name} for ${reward.cost} EdCoins`);
      ...
      alert(`Successfully redeemed ${reward.name}! Your new balance is ${userCoins - reward.cost} EdCoins.`); // Replace with better UI feedback
  ...
  if (isLoading) {
    return (
        <div className="flex justify-center items-center min-h-[calc(100vh-8rem)] bg-white text-slate-600"> {/* Adjust min-height based on nav/footer */}
  ...
  if (error) {
    return <div className="flex justify-center items-center min-h-[calc(100vh-8rem)] bg-white text-red-600">{error}</div>;
  }

  return (
    <div className="min-h-screen bg-white text-slate-800 font-sans pt-20 pb-16"> {/* Adjust pt-20 if you have a fixed navbar */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
            {rewards.map((reward, index) => {
              ...
              return (
                ...
                >
                  {/* Reward Image */}
                  <div className="aspect-w-16 aspect-h-9"> {/* Or choose aspect ratio */}
                     <img
                        src={reward.imageUrl}
                        alt={reward.name}
                  ...
                  <div className="p-4 flex flex-col flex-grow">
                    <h3 className="text-lg font-semibold text-slate-800 mb-1 truncate" title={reward.name}>
                      {reward.name}
                    </h3>
                    ...
                    <div className="flex justify-between items-center mt-auto pt-3 border-t border-slate-200">
                      <div className="flex items-center">
                        ...
                        <span className="text-xs text-slate-500 ml-1">Coins</span>
                      </div>
                      <button
